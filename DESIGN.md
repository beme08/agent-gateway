...
> **Source**: [getdesign.md/intercom](https://getdesign.md/intercom/design-md) — adapted for our HR-grade agent gateway. The full Intercom analysis is preserved below for reference; the design tokens live in `apps/web/tailwind.config.js` and `apps/web/app/globals.css`.

version: alpha
name: Agent-Gateway-design-system (Intercom-inspired)
